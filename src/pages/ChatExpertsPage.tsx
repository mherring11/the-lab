...
export default function ChatExpertsPage() {
  ...
  const handleSelectExpert = (expertId: string) => {
    ...
    setLastAccessTimes(prev => {
      ...
      localStorage.setItem('expertAccessTimes', JSON.stringify(
  ...
  const formatLastAccess = (time?: Date) => {
    if (!time) return 'New conversation';
  ...
  return (
    <div className="flex h-screen bg-gray-50">
      {/* Sidebar with experts list */}
      <div className="w-80 bg-white border-r border-gray-200 flex flex-col">
        <div className="p-4 border-b border-gray-200">
          <h2 className="text-xl font-semibold text-gray-800">Health Experts</h2>
          <p className="text-sm text-gray-500 mt-1">Your personal AI coaching team</p>
          ...
          <div className="mt-3 relative">
            ...
            <input
              ...
              className="block w-full pl-10 pr-3 py-2 border border-gray-300 rounded-md text-sm placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-yellow-500 focus:border-yellow-500"
              value={searchQuery}
            ...
            filteredExperts.map(expert => (
              <div
                ...
                className={`p-4 border-b border-gray-100 cursor-pointer hover:bg-gray-50 transition-colors ${
                  selectedExpert === expert.id ? 'bg-yellow-50' : ''
                }`}
              >
                <div className="flex items-center">
                  <div className="flex-shrink-0 h-12 w-12 rounded-full bg-gradient-to-br from-yellow-400 to-yellow-500 flex items-center justify-center text-2xl shadow-sm">
                    {expert.avatar}
                  </div>
                  <div className="ml-4 flex-1">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-xs text-gray-500">
                        {formatLastAccess(lastAccessTimes[expert.id])}
                      </span>
                    </div>
                    <p className="text-xs text-gray-500">{expert.title}</p>
                    <div className="mt-1">
                      <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${
                        expert.category === CATEGORIES.TRAINING ? 'bg-blue-100 text-blue-800' :
      ...
      <div className="flex-1 flex flex-col">
        {selectedExpert ? (
          <ChatWithExpert 
            ...
            expert={AI_EXPERTS.find(e => e.id === selectedExpert)!}
          />
        ) : (
          <div className="flex-1 flex flex-col items-center justify-center bg-gray-50 p-8 text-center">
            <div className="h-24 w-24 rounded-full bg-yellow-100 flex items-center justify-center mb-6">
              <span className="text-4xl">👋</span>
            </div>
            <h2 className="text-2xl font-bold text-gray-900 mb-2">Meet Your Expert Team</h2>
            <p className="text-gray-600 max-w-md mb-6">
              Select an expert from the sidebar to get personalized guidance for your fitness journey
            </p>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 max-w-3xl">
              {Object.values(CATEGORIES).filter(cat => cat !== CATEGORIES.ALL).map(category => (
                <button
                  key={category}
                  onClick={() => setActiveCategory(category)}
                  className="p-4 border border-gray-200 rounded-lg bg-white hover:bg-gray-50 text-center"
                >
                  <p className="font-medium text-gray-900 mb-1">{category}</p>
                  <p className="text-xs text-gray-500">
                    {AI_EXPERTS.filter(e => e.category === category).length} experts
                  </p>
                </button>
              ))}
            </div>
