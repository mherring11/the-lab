...
import { 
  Send, Paperclip, Smile, Info, 
  ChevronDown, X, Bookmark, BookmarkCheck,
...
const ChatWithExpert: React.FC<ChatWithExpertProps> = ({ expertId, expert }) => {
  ...
  const generateAIResponse = (message: string, expert: Expert): string => {
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center p-4 border-b border-gray-200 bg-white">
        <img 
          ...
          onError={(e) => {
            (e.target as HTMLImageElement).src = `https://ui-avatars.com/api/?name=${encodeURIComponent(expert.name)}&background=F59E0B&color=fff`;
          }}
        />
        <div className="ml-3">
      ...
      {showInfo && (
        <div className="p-4 bg-gray-50 border-b border-gray-200">
          <div className="flex justify-between items-center mb-2">
            <h4 className="font-medium">About {expert.name}</h4>
            <button
          ...
          <div className="flex items-center">
            <span className="text-xs py-1 px-2 bg-gray-200 text-gray-700 rounded-full">
              {expert.category.charAt(0).toUpperCase() + expert.category.slice(1)}
            </span>
          </div>
        </div>
      )}
        ...
        {Object.entries(groupedMessages).map(([date, dateMessages]) => (
          <div key={date}>
            <div className="relative flex items-center my-4">
              ...
              <span className="flex-shrink-0 mx-4 text-xs text-gray-500">{date}</span>
              <div className="flex-grow border-t border-gray-200"></div>
            </div>
              ...
              {dateMessages.map((message) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className={`flex max-w-[75%] ${isUserMessage ? 'flex-row-reverse' : 'flex-row'}`}>
                      {/* Message content */}
                      <div
                      ...
                      {/* User avatar (appears on right of message) */}
                      {isUserMessage && (
      ...
      {/* Display selected attachments before sending */}
      ...
      >
        ...
        <input 
          type="text" 
          className="flex-1 mx-3 px-4 py-2 border rounded-full text-sm text-gray-900"
        ...
        <button 
          type="submit" 
          className="p-2 ml-1 rounded-full bg-yellow-500 text-white hover:bg-yellow-600"
        >
